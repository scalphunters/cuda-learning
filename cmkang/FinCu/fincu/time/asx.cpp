...
namespace FinCu {
    ...
    std::string ASX::code(const Date& date) {
        FC_REQUIRE(isASXdate(date, false),
        ...
        switch(date.month()) {
          ...
          case December:
            ...
            break;
          default:
            FC_FAIL("not an ASX month (and it should have been)");
        }
    ...
    Date ASX::date(const std::string& asxCode,
                   const Date& refDate) {
        FC_REQUIRE(isASXcode(asxCode, false),
        ...
        else FC_FAIL("invalid ASX month letter");
