...
#include <fincu/utilities/dataformatters.hpp>
...
namespace FinCu {
    ...
    bool Date::isLeap(Year y) {
        ...
        FC_REQUIRE(y>=1900 && y<=2200, "year outside valid range");
    ...
    void Date::checkSerialNumber(BigInteger serialNumber) {
        FC_REQUIRE(serialNumber >= minimumSerialNumber() &&
    ...
    std::ostream& operator<<(std::ostream& out, Month m) {
        switch (m) {
          ...
          case December:
            return out << "December";
          default:
            FC_FAIL("unknown month (" << Integer(m) << ")");
        }
    }
