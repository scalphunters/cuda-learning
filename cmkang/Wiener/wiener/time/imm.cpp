...
namespace Wiener {
    ...
    std::string IMM::code(const Date& date) {
        WI_REQUIRE(isIMMdate(date, false),
        ...
        switch(date.month()) {
          ...
          case December:
            ...
            break;
          default:
            WI_FAIL("not an IMM month (and it should have been)");
        }
    ...
    Date IMM::date(const std::string& immCode,
                   const Date& refDate) {
        WI_REQUIRE(isIMMcode(immCode, false),
                   immCode << " is not a valid IMM code");
		WI_REQUIRE(refDate != Date(), "null date given");
        ...
        else WI_FAIL("invalid IMM month letter");
...
//        Year y = boost::lexical_cast<Year>(); // lexical_cast causes compilation errors with x64
    ...
    Date IMM::nextDate(const Date& date, bool mainCycle) {
		WI_REQUIRE(date != Date(), "null date given");
        Date refDate = date;
        Year y = refDate.year();
