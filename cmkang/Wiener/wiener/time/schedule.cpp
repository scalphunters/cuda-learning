...
namespace Wiener {
    ...
    {
        // sanity checks
		WI_REQUIRE(effectiveDate != Date(), "null effective date");
        WI_REQUIRE(terminationDate != Date(), "null termination date");

        WI_REQUIRE(effectiveDate < terminationDate,
        ...
        if (firstDate_ != Date()) {
            switch (rule_) {
              ...
              case DateGeneration::Forward:
                WI_REQUIRE(firstDate_ > effectiveDate &&
              ...
              case DateGeneration::ThirdWednesday:
                  WI_REQUIRE(IMM::isIMMdate(firstDate_, false),
                             "first date (" << firstDate_ <<
                             ") is not an IMM date");
                break;
              ...
              case DateGeneration::CDS:
                WI_FAIL("first date incompatible with " << rule_ <<
                        " date generation rule");
              default:
                WI_FAIL("unknown rule (" << Integer(rule_) << ")");
            }
        }
        if (nextToLastDate_ != Date()) {
            switch (rule_) {
              case DateGeneration::Backward:
              case DateGeneration::Forward:
                WI_REQUIRE(nextToLastDate_ > effectiveDate &&
                ...
                break;
              case DateGeneration::ThirdWednesday:
                WI_REQUIRE(IMM::isIMMdate(nextToLastDate_, false),
                           "next-to-last date (" << nextToLastDate_ <<
                           ") is not an IMM date");
                break;
              case DateGeneration::Zero:
              ...
              case DateGeneration::OldCDS:
              case DateGeneration::CDS:
                WI_FAIL("next to last date incompatible with " << rule_ <<
                        " date generation rule");
              default:
                WI_FAIL("unknown rule (" << Integer(rule_) << ")");
            }
    ...
    Schedule Schedule::until(const Date& truncationDate) const {
        ...
        WI_REQUIRE(truncationDate>result.dates_[0],
    ...
    bool Schedule::isRegular(Size i) const {
        WI_REQUIRE(isRegular_.size() > 0,
                   "full interface (isRegular) not available");
        WI_REQUIRE(i<=isRegular_.size() && i>0,
    ...
    MakeSchedule::operator Schedule() const {
        ...
        WI_REQUIRE(effectiveDate_ != Date(), "effective date not provided");
        WI_REQUIRE(terminationDate_ != Date(), "termination date not provided");
