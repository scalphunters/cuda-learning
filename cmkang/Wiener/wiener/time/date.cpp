...
namespace Wiener {
    ...
    bool Date::isLeap(Year y) {
        ...
        WI_REQUIRE(y>=1900 && y<=2200, "year outside valid range");
    ...
    void Date::checkSerialNumber(BigInteger serialNumber) {
        WI_REQUIRE(serialNumber >= minimumSerialNumber() &&
    ...
    std::ostream& operator<<(std::ostream& out, Month m) {
        switch (m) {
          ...
          case December:
            return out << "December";
          default:
            WI_FAIL("unknown month (" << Integer(m) << ")");
        }
    }
