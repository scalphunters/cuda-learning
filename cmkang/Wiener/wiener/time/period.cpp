...
namespace Wiener {
    ...
    Period::Period(Frequency f) {
        switch (f) {
          ...
          case Daily:
            ...
            break;
          case OtherFrequency:
            WI_FAIL("unknown frequency");  // no point in showing 999...
          default:
        ...
        switch (units_) {
          ...
          case Days:
            if (length==1)
                ...
                return OtherFrequency;
          default:
            WI_FAIL("unknown time unit (" << Integer(units_) << ")");
        }
    }
            ...
            switch (units_) {
              ...
              case Years:
                switch (p.units()) {
                  ...
                  case Weeks:
                  case Days:
                    WI_REQUIRE(p.length()==0,
                               "impossible addition between " << *this <<
                               " and " << p);
                    break;
                  default:
                    WI_FAIL("unknown time unit (" << Integer(p.units()) << ")");
                }
                break;
    ...
    Period& Period::operator/=(Integer n) {
        WI_REQUIRE(n != 0, "cannot be divided by zero");
        if (length_ % n == 0) {
    ...
    Real years(const Period& p) {
        ...
        switch (p.units()) {
          case Days:
            WI_FAIL("cannot convert Days into Years");
          case Weeks:
            WI_FAIL("cannot convert Weeks into Years");
          case Months:
    ...
    Real months(const Period& p) {
        ...
        switch (p.units()) {
          case Days:
            WI_FAIL("cannot convert Days into Months");
          case Weeks:
            WI_FAIL("cannot convert Weeks into Months");
          case Months:
              return p.length();
          case Years:
              return p.length()*12.0;
    ...
    Real weeks(const Period& p) {
        ...
        switch (p.units()) {
          ...
          case Weeks:
              return p.length();
          case Months:
            WI_FAIL("cannot convert Months into Weeks");
          case Years:
            WI_FAIL("cannot convert Years into Weeks");
          default:
            WI_FAIL("unknown time unit (" << Integer(p.units()) << ")");
        }
    }
    ...
    Real days(const Period& p) {
        ...
        switch (p.units()) {
          ...
          case Weeks:
              return p.length()*7.0;
          case Months:
            WI_FAIL("cannot convert Months into Days");
          case Years:
            WI_FAIL("cannot convert Years into Days");
          default:
            WI_FAIL("unknown time unit (" << Integer(p.units()) << ")");
        }
    }
    ...
    bool operator<(const Period& p1, const Period& p2) {
        ...
        if (p1lim.second < p2lim.first)
            ...
            WI_FAIL("undecidable comparison between " << p1 << " and " << p2);
    }
    ...
    namespace detail {
        ...
        std::ostream& operator<<(std::ostream& out,
            ...
            switch (holder.p.units()) {
              ...
              default:
                WI_FAIL("unknown time unit (" << Integer(holder.p.units()) << ")");
            }
        }
